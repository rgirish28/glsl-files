...
// Important! Two textures.
uniform sampler2D texture_map, bump_map;
...
void main()
{
  ...
  vec3 tex_val = texture2D(texture_map, texcoord).xyz;
  ...
  vec3 ambient_color = vec3(0) * light_color;
  vec3 diffuse_color = tex_val * light_color * max(n_dot_l,0.f);
  vec3 specular_color = tex_val * light_color 
